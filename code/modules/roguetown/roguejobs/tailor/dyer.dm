...
/obj/machinery/gear_painter/Topic(href, href_list)
	...
	if(href_list["select"])
		...
		if(alert(usr, "Input Choice", "Primary Dye", "Color Wheel", "Color Preset") != "Color Wheel")
			choice = input(usr, "Choose your dye:", "Dyes", null) as null|anything in colorlist
			if(!choice)
				return
			activecolor = colorlist[choice]
		else
			activecolor = sanitize_hexcolor(color_pick_sanitized_lumi(usr, "Choose your dye:", "Dyes", choice ? choice : activecolor_detail), 6, TRUE)
		updateUsrDialog()

	if(href_list["select_detail"])
		var/choice
		if(alert(usr, "Input Choice", "Secondary Dye", "Color Wheel", "Color Preset") != "Color Wheel")
			choice = input(usr, "Choose your dye:", "Dyes", null) as anything in colorlist|null
			if(!choice)
				return
			activecolor_detail = colorlist[choice]
		else
			activecolor_detail = sanitize_hexcolor(color_pick_sanitized_lumi(usr, "Choose your dye:", "Dyes", choice ? choice : activecolor_detail), 6, TRUE)
		updateUsrDialog()
	...
	if(href_list["select_altdetail"])
		...
		if(alert(usr, "Input Choice", "Tertiary Dye", "Color Wheel", "Color Preset") != "Color Wheel")
			...
			activecolor_altdetail = sanitize_hexcolor(color_pick_sanitized_lumi(usr, "Choose your dye:", "Dyes", choice ? choice : activecolor_detail), 6, TRUE)
		updateUsrDialog()
...
/obj/item/dye_brush/attack_self(mob/user)
	...
	var/hexdye
	if(!dye)
		hexdye = sanitize_hexcolor(color_pick_sanitized_lumi(usr, "Choose your dye:", "Dyes", null, 0.09), 6, TRUE)
		if (hexdye == "#000000")
			return
		dye = hexdye
		update_icon()
