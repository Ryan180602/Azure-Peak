...
/obj/structure/roguemachine/mail/proc/ghost_cooldown(mob/user)
	if(!user || !user.ckey)
		return 0
	var/last = ghost_sends[user.ckey]
	if(!last)
		return 0
	return max(0, (last + GHOST_LETTER_COOLDOWN) - world.time)
...
/obj/structure/roguemachine/mail/proc/read_mail(mob/user)
	...
	for(var/i = length(box), i >= 1, i--)
		...
		dat += "<b>From [GL.sender], to [GL.recipient]</b><br>"
		dat += GL.content
		GL.unread = FALSE
		if(i > 1)
			dat += "<hr>"
	dat += "</body></html>"
	user << browse(dat, "window=ghostmail;size=500x400;can_close=1;can_minimize=0;can_maximize=0;can_resize=1;titlebar=1;border=0")
	return TRUE
...
/obj/structure/roguemachine/mail/ui_data(mob/user)
	...
	data["letter_count"] = length(ghost_mailbox(user))
	return data

/obj/structure/roguemachine/mail/proc/log_mail_send(mob/user, sender_name, recipient_name, ghost_send = FALSE)
	if(!user)
		return
	...
	message_admins("[key_name(user)] [what] via [name]/[(loc)] from [sender_name] to [recipient_name]")
...
/obj/structure/roguemachine/mail/proc/ghost_post(mob/dead/observer/user, list/params)
	...
	if(!mail_exists(target))
		var/fail_msg = findtext(target, "#") ? "There is no such box to send it to." : "There's no one by that name to receive it."
		to_chat(user, span_warning(fail_msg))
		qdel(P)
		return FALSE
	SSroguemachine.pending_letters += new /datum/pending_mail(P, target, sender, user.ckey, rand(GHOST_LETTER_DELAY_MIN, GHOST_LETTER_DELAY_MAX))
	ghost_sends[user.ckey] = world.time
	log_mail_send(user, sender, target, TRUE)
	...
	switch(action)
		...
		if("send_letter")
			...
			if(!mail_exists(send2place))
				var/fail_msg = findtext(send2place, "#") ? "Failed to send. Bad number?" : "There's no one by that name to receive it."
				to_chat(user, span_warning(fail_msg))
				qdel(P)
				return TRUE
	...
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/smallDelivery))
		...
		if(alert(user, "Send Mail?",,"YES","NO") == "YES")
			...
			if(!send2place)
				return
			if(!mail_exists(send2place))
				var/fail_msg = findtext(send2place, "#") ? "Cannot send it. Bad number?" : "There's no one by that name to receive it."
				to_chat(user, span_warning(fail_msg))
				return
			if(!deliver_mail(P, send2place, sentfrom))
				to_chat(user, span_warning("The master of mails has perished?"))
