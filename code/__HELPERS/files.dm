...
/proc/music_upload(mob/user, title = "Music", max_size = MUSIC_MAX_SIZE)
	var/ckey = user?.ckey
	if(!ckey)
		return
	...
	var/infile = input(user, "CHOOSE A NEW SONG", "[title]") as null|file

	if(uploader)
		uploader.upload_limit = null
		uploader.upload_exts = null

	if(!infile || QDELETED(user) || user.ckey != ckey)
		return
